...
function LenghtValid()
{
  
  //alert(document.getElementById("txtContact").value.length);
  var len = document.getElementById("txtContact").value.length;
  if(len<10)
  {
...
function CharsOnly(e)
{
  ...
  if (Unicode!=8 )
  { //if the key isn't the backspace key (which we should allow)
   if (Unicode>47 && (Unicode<65 || Unicode>90) && (Unicode<97 || Unicode>122) ) //if not a number
   {
      return false; //disable key press  
   }
...
function ValidateFileUpload(ControlId,LabelID)
{
        var imgfile = document.getElementById(ControlId);
        var FileUploadPath = imgfile.value;

        //To check if user upload any file
        if (FileUploadPath == '')
        {
            document.getElementById(LabelID).innerHTML = "* Please upload an image";
            return false;
        }
        else
        {
        ...
        //The file upload is NOT an image
            ...
            {
                document.getElementById(LabelID).innerHTML = " *Photo only allows file types of PNG, JPG, JPEG  ";
                return false;
            }
        }
    }